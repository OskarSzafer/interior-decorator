...
std::vector<Furniture> readCSV(const std::string& filename) {
	...
	while (std::getline(file, line)) {
		...
		std::getline(ss, item, 'x');
		furniture.position[2] = std::stof(item);

		// Parse angle
		std::getline(s, item, ',');
		furniture.angle = std::stof(item);
		...
		std::getline(sb, item, 'x');
		furniture.rotation[2] = std::stof(item);
...
void printRecords(const std::vector<Furniture>& furnitureList) {
	for (const auto& record : furnitureList) {
		std::cout << "Name: " << record.type
			<< ", PositonX: " << record.position[0]
			<< ", PositonY: " << record.position[1]
			<< ", PositonZ: " << record.position[2]
			<< ", Angle: " << record.angle
			<< ", RotationX: " << record.rotation[0]
			<< ", RotationY: " << record.rotation[1]
			<< ", RotationZ: " << record.rotation[2]
			<< ", Scale: " << record.scale
			<< std::endl;
...
void initOpenGLProgram(GLFWwindow* window) {
	...
	model2.readTexture("OBJ/TX_Table_1_1_Base_color.png"); // stolik + krzeslo
	model2.loadModel("OBJ/Table_Chair_1.obj");

	model3.readTexture("OBJ/Masa.png"); // stolik okragly
	model3.loadModel("OBJ/Masa.fbx");

	model4.readTexture("OBJ/bench.png"); // lawka
	model4.loadModel("OBJ/bench.fbx");

	model5.readTexture("OBJ/TX_Table_1_1_Base_color.png"); // floor
	model5.loadModel("OBJ/tf.fbx");

	model6.readTexture("OBJ/1.png"); // sofa
	model6.loadModel("OBJ/1.obj");

	model7.readTexture("OBJ/wall.png"); // wall
	model7.loadModel("OBJ/wall.fbx");
...
void drawScene(GLFWwindow* window) {
	...
	//floor
	model5.drawModel(P, V, M, glm::vec3(0.0f, -0.48f, -0.425f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.0f, -0.48f, 0.0f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.0f, -0.48f, 0.425f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.0f, -0.48f, 0.85f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));

	model5.drawModel(P, V, M, glm::vec3(0.625f, -0.48f, -0.425f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.625f, -0.48f, 0.0f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.625f, -0.48f, 0.425f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	model5.drawModel(P, V, M, glm::vec3(0.625f, -0.48f, 0.85f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	...
	//wall
	model7.drawModel(P, V, M, glm::vec3(0.0f, -0.2f, -0.6f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.38f, 0.5f, 0.45f));
	glm::mat4 MR = glm::rotate(M, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	model7.drawModel(P, V, MR, glm::vec3(0.12f, -0.2f, -0.95f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.38f, 0.5f, 0.45f));
	...
	for (const auto& record : furnitureList) {
		if (record.type == "SOFA") {
			model6.drawModel(P, V, M, glm::vec3(record.position[0], record.position[1], record.position[2]), glm::radians(record.angle), glm::vec3(record.rotation[0], record.rotation[1], record.rotation[2]), glm::vec3(record.scale, record.scale, record.scale));
		}
		else if (record.type == "CHAIR_AND_TABLE") {
			model2.drawModel(P, V, M, glm::vec3(record.position[0], record.position[1], record.position[2]), glm::radians(record.angle), glm::vec3(record.rotation[0], record.rotation[1], record.rotation[2]), glm::vec3(record.scale, record.scale, record.scale));
		}
		else if (record.type == "MUG") {
			model1.drawModel(P, V, M, glm::vec3(record.position[0], record.position[1], record.position[2]), glm::radians(record.angle), glm::vec3(record.rotation[0], record.rotation[1], record.rotation[2]), glm::vec3(record.scale, record.scale, record.scale));
		}
		else if (record.type == "BENCH") {
			model4.drawModel(P, V, M, glm::vec3(record.position[0], record.position[1], record.position[2]), glm::radians(record.angle), glm::vec3(record.rotation[0], record.rotation[1], record.rotation[2]), glm::vec3(record.scale, record.scale, record.scale));
		}
		else if (record.type == "ROUND_TABLE") {
			model3.drawModel(P, V, M, glm::vec3(record.position[0], record.position[1], record.position[2]), glm::radians(record.angle), glm::vec3(record.rotation[0], record.rotation[1], record.rotation[2]), glm::vec3(record.scale, record.scale, record.scale));
		}
	}

	//model4.drawModel(P, V, M, glm::vec3(0.0f, 0.0f, -0.45f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.005f, 0.005f, 0.005f));
	//model4.drawModel(P, V, M, glm::vec3(0.0f, 0.0f, 0.9f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.005f, 0.005f, 0.005f));

	//model6.drawModel(P, V, M, glm::vec3(0.45f, -0.13f, 0.0f), glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.005f, 0.005f, 0.005f));
